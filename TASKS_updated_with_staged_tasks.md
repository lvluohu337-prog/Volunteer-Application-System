...
- 河南相关真实招生表已经完成导入，并已验证 `backend/planning_repository.py` 的推荐 / 报告链路可以稳定命中真实候选。
- 当前正式支持省份已收口为 `河南`，并且已经落成共享定义；浙江、河北、山东、江苏、安徽、广东、四川已完成目录级核验，结论是“已有标准化素材，但尚未形成与河南同等级别的正式导入闭环”。

当前最需要正视的不是“页面还不够多”，而是“设置页最小可用、正式支持省份边界、解释库能力和深度版能力还没有收口”。

当前最大的 6 个缺口：

1. `src/pages/SettingsPage.vue` 仍然是占位页，缺少正式系统最小可用的配置能力。
2. 浙江等目标省份虽已有部分标准化素材，但仍未形成与河南同等级别的导入产物、专项导入脚本和正式推荐/报告闭环。
3. 前端关键链路仍有静默 fallback / demo 回退，需要继续收口错误处理边界。
4. 本地专业解释库、城市解释库、画像映射库和报告话术库还未建立，解释层价值仍偏弱。
5. 仓库中仍保留 demo / base-data / legacy / SQLite 等历史痕迹，工程口径还需要继续收敛。
6. `699` 深度版能力尚未产品化，多方案对比、考研路径、就业迁移和人工复核入口都还未落地。
...
- [x] 统一当前正式产品版本口径为一套可执行定义
- [x] 修正工作台中的 `99 / 399 / 699 / 999` 文案
- [x] 修正学生详情页中的 `99 / 399 / 699 / 999` 文案
- [x] 修正报告页、文档、产品说明中的版本描述
- [x] 明确 `699` 是暂不支持还是立即补做
...
1. 当前正式支持省份边界已经明确为 `河南`；浙江、河北、山东、江苏、安徽、广东、四川虽已有标准化素材，但仍未形成与河南同等级别的正式导入闭环。
2. 前端关键流程的静默 fallback 还没有完全收口，部分接口仍需要改成显式报错和可执行提示。
3. `src/pages/SettingsPage.vue` 还不是正式可用后台，缺少顾问署名、合规文案、导出基础配置和产品展示配置。
4. 本地专业 / 城市 / 画像 / 话术解释库尚未建立，当前 399 元标准版的解释层说服力还有提升空间。
5. 仓库里仍有 `backend/*.db` 级别的本地 SQLite 痕迹，和“统一 PostgreSQL”表述并不完全一致。
6. `data_assets/generated_reports/` 当前没有在 `.gitignore` 中统一忽略，且历史产物格式混杂，需要明确管理策略。
7. demo / base-data / settings 等页面仍在代码中存在，只是主导航弱化了入口。
