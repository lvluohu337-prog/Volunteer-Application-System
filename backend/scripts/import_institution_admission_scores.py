...
HENAN_HISTORY_ROOT = PROJECT_ROOT / "河南-2026志愿填报资料" / "3、河南高考历史数据" / "河南_投档线"
IMPORTED_DIR = PROJECT_ROOT / "data_assets" / "imported" / "henan_institution_admission_scores"
SOURCE_YEARS = [2017, 2018, 2019, 2020, 2021]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import Henan institution admission scores datasets into the planning database."
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _institution_row(row: dict[str, str]) -> dict[str, object] | None:
    ...
    if not (exam_year and institution_name and subject_track and batch_code and min_score is not None):
        return None
    return {
        "exam_year": exam_year,
        "province": "河南",
        ...
        "planned_count": None,
        "admitted_count": None,
        "score_line": _parse_float(row.get("省控线")),
...
def load_henan_historical_institution_scores(
    ...
    for year in SOURCE_YEARS:
        path = HENAN_HISTORY_ROOT / f"河南_投档线_{year}.xlsx"
        normalized_count = 0
        for _, row in iter_sheet_records(path, limit=limit_per_file):
            ...
            normalized = _institution_row(row)
            if not normalized:
                continue
    ...
    datasets = {
        ...
        "institution_admission_scores": rows,
    }
    metadata = {
...
def main() -> None:
    ...
    datasets, metadata = load_henan_historical_institution_scores(limit_per_file=args.limit)
    _write_normalized_files(datasets)

    summary: dict[str, object] = {
        "scope": "henan_institution_admission_scores",
    ...
    if not args.dry_run:
        import_counts = {
            ...
            "institution_admission_scores": upsert_institution_admission_scores(
                datasets["institution_admission_scores"]
            ),
        }
        summary["import_counts"] = import_counts
        summary["schema_overview"] = get_admissions_schema_overview()

    write_json(IMPORTED_DIR / "henan_institution_admission_scores_import_summary.json", summary)

    print("=== HENAN INSTITUTION ADMISSION SCORES IMPORT SUMMARY ===")
