...
$checkScript = Join-Path $projectRoot "scripts\check_product_flow_e2e.cjs"
...
if (-not (Test-Path $checkScript)) {
    throw "Product flow E2E script not found: $checkScript"
}
...
Write-Host "Building frontend for product-flow E2E..."
& $nodeExe $viteCli build
if ($LASTEXITCODE -ne 0) {
    throw "Frontend build failed."
}

Write-Host "Running product-flow E2E regression..."
& $nodeExe $checkScript
if ($LASTEXITCODE -ne 0) {
    throw "Product-flow E2E regression failed."
}
